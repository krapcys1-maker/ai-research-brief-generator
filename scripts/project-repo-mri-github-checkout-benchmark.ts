...
const jsonOutputPath =
  process.env.PROJECT_REPO_MRI_GITHUB_BENCHMARK_JSON ??
  "benchmark-results/project-repo-mri-github-checkout-latest.json";
const markdownOutputPath =
  process.env.PROJECT_REPO_MRI_GITHUB_BENCHMARK_MARKDOWN ??
  "benchmark-results/project-repo-mri-github-checkout-latest.md";
const githubRepoUrl =
  process.env.PROJECT_REPO_MRI_GITHUB_REPO_URL ??
  "https://github.com/krapcys1-maker/ai-research-brief-generator.git";
const githubRef =
  process.env.PROJECT_REPO_MRI_GITHUB_REF ??
  "codex/project-research-plan-benchmark";
...
const repoMriIdea: ProjectIdeaInput = {
  ...
  constraints: [
    ...
    "benchmark Bug Path on shallow GitHub checkouts as an optional live gate"
  ],
...
const cases: GithubCheckoutCase[] = [
  {
  ...
  return researchPlan.evidenceBuckets.flatMap((bucket, bucketIndex) =>
    [0, 1].map((offset) => {
      ...
      return {
        ...
        limitations: ["fixture research for repeatable GitHub-checkout benchmark"],
...
function runCommand(input: { cwd: string; command: string; args: string[] }) {
  ...
  if (result.status !== 0) {
    throw new Error(
      [
        `${input.command} ${input.args.join(" ")} failed with ${result.status}`,
        result.stdout,
        result.stderr
      ].join("\n")
    );
...
function runPythonJson<T>(input: { cwd: string; args: string[] }) {
  return JSON.parse(runCommand({ cwd: input.cwd, command: "python", args: input.args })) as T;
}
...
async function generateRepoMriPack(outputDir: string) {
  await runProjectResearch({
    ...
    generatedAt: "2026-06-04T10:50:00.000Z",
    outputDir
  });
...
function cloneGithubRepo(root: string) {
  const repoDir = join(root, "github_checkout");
  runCommand({
    cwd: root,
    command: "git",
    args: [
...
function renderMarkdownReport(input: {
  ...
  const lines = [
    "# Repo MRI GitHub Checkout Benchmark",
    "",
...
async function main() {
  ...
  const root = await mkdtemp(join(tmpdir(), "repo-mri-github-checkout-benchmark-"));
  const packOutputDir =
    process.env.PROJECT_REPO_MRI_GITHUB_PACK_DIR ?? join(root, "system_output");
  const dbPath = join(root, "github_checkout.sqlite");
  const { repoDir, commit } = cloneGithubRepo(root);
