  ...
  return [
    ...
    },
    {
      ...
      content: [
        ...
        "",
        "",
        "def extract_terms(issue: str) -> tuple[list[tuple[str, int | None]], list[str], list[str]]:",
        ...
        "    primary_issue = re.split(r'\\b(?:while|but|although|whereas)\\b', issue, maxsplit=1, flags=re.IGNORECASE)[0]",
        "    primary_terms = [word for word in WORD_RE.findall(primary_issue) if word.lower() not in stop]",
        "    return files, sorted(set(terms), key=str.lower), sorted(set(primary_terms), key=str.lower)",
        "",
        "",
        "def bug_path(db_path: Path, issue: str, limit: int = 8) -> dict:",
        "    conn = connect(db_path)",
        "    files, terms, primary_terms = extract_terms(issue)",
        ...
        "",
        "    def test_command(path: str, symbol: str | None = None) -> str:",
        "        if path.endswith('.py'):",
        "            if symbol and symbol.startswith('test'):",
        "                return f'pytest {path}::{symbol}'",
        "            return f'pytest {path}'",
        "        if path.endswith(('.ts', '.tsx', '.js', '.jsx')):",
        "            return f'npm test -- {path}'",
        "        return f'run tests for {path}'",
        "",
        "    def characterization_test_for(item: dict) -> dict:",
        "        file_name = item['path'].split('/')[-1]",
        "        base_name = file_name.rsplit('.', 1)[0]",
        "        symbol = item.get('symbol') or base_name",
        "        if item['path'].endswith('.py'):",
        "            test_path = f'tests/test_{base_name}.py'",
        "            test_name = f'test_{symbol}_characterization'",
        "            repro_input = f\"minimal input from issue: {issue}\"",
        "            expected_assertion = 'assert current failing behavior is reproduced before editing'",
        "            lower_terms = {term.lower() for term in terms}",
        "            if symbol == 'is_feature_enabled' and {'expired', 'expires'} & lower_terms:",
        "                test_name = 'test_is_feature_enabled_expired_flag_is_disabled'",
        "                repro_input = \"flag={'accounts':['acct_1'],'expires_at': datetime(2026, 1, 1)}, account_id='acct_1', now=datetime(2026, 1, 2)\"",
        "                expected_assertion = \"assert is_feature_enabled(flag, 'acct_1', now) is False\"",
        "            return {",
        "                'path': test_path,",
        "                'name': test_name,",
        "                'command': test_command(test_path, test_name),",
        "                'input': repro_input,",
        "                'assertion': expected_assertion,",
        "            }",
        "        test_path = f'tests/{base_name}.test.ts'",
        "        test_name = f'{symbol} characterization'",
        "        return {",
        "            'path': test_path,",
        "            'name': test_name,",
        "            'command': test_command(test_path),",
        "            'input': f'minimal input from issue: {issue}',",
        "            'assertion': 'assert current failing behavior is reproduced before editing',",
        "        }",
        "",
        "    def related_tests_for(item: dict) -> list[dict]:",
        "        base_name = item['path'].split('/')[-1].rsplit('.', 1)[0]",
        "        probes = [item.get('symbol') or '', base_name, *terms[:8]]",
        "        test_candidates: dict[str, dict] = {}",
        "        for probe in sorted({probe for probe in probes if probe}, key=str.lower):",
        "            like = f'%{probe}%'",
        "            for row in conn.execute(",
        "                \"\"\"",
        "                SELECT f.path, s.name AS symbol_name, c.start_line, c.end_line, c.content",
        "                FROM chunks c",
        "                JOIN files f ON f.id = c.file_id",
        "                LEFT JOIN symbols s ON s.id = c.symbol_id",
        "                WHERE (LOWER(f.path) LIKE '%test%' OR LOWER(f.path) LIKE '%spec%')",
        "                  AND c.content LIKE ?",
        "                LIMIT 25",
        "                \"\"\"",
        "                , (like,),",
        "            ):",
        "                key = f\"{row['path']}::{row['symbol_name'] or ''}\"",
        "                test = test_candidates.setdefault(key, {",
        "                    'path': row['path'],",
        "                    'symbol': row['symbol_name'],",
        "                    'score': 0.0,",
        "                    'line_range': [row['start_line'], row['end_line']],",
        "                    'evidence': [],",
        "                    'command': test_command(row['path'], row['symbol_name']),",
        "                })",
        "                points = 3.0 if item.get('symbol') and probe.lower() == item['symbol'].lower() else 1.0",
        "                test['score'] += points",
        "                if len(test['evidence']) < 4:",
        "                    test['evidence'].append(f'test content match: {probe}')",
        "        def test_rank(test: dict) -> tuple[float, int]:",
        "            symbol = test.get('symbol') or ''",
        "            specificity = 1 if symbol.startswith('test') else 0",
        "            return (test['score'], specificity)",
        "        return sorted(test_candidates.values(), key=test_rank, reverse=True)[:3]",
        "",
        "    def build_unknowns(ranked: list[dict]) -> list[str]:",
        "        unknowns = [",
        "            'Static localization cannot confirm runtime values, production state or exact failing input without a reproducer.',",
        "            'Ranking is based on indexed code text, symbols, line hints and tests; it has not executed the application.',",
        "        ]",
        "        if not files:",
        "            unknowns.append('Issue text did not include an explicit file path, so file localization is inferred from terms and symbols.')",
        "        if ranked and not ranked[0].get('related_tests'):",
        "            unknowns.append('Top candidate has no directly matched related test in the indexed repository.')",
        "        if ranked and any('call graph:' in evidence for evidence in ranked[0]['evidence']):",
        "            unknowns.append('Top candidate includes heuristic CALLS graph evidence; verify the runtime path with a focused test.')",
        "        if len(ranked) > 1:",
        "            top_score = ranked[0]['score']",
        "            runner_up_gap = top_score - ranked[1]['score']",
        "            close_threshold = max(2.0, top_score * 0.2)",
        "            if runner_up_gap <= close_threshold:",
        "                unknowns.append('Top candidates are close in score; inspect runner-up evidence before editing.')",
        "        return unknowns",
        "",
        "    def why_not_other_candidates(ranked: list[dict]) -> list[dict]:",
        "        if not ranked:",
        "            return []",
        "        top = ranked[0]",
        "        decisions = []",
        "        top_primary_hits = sum(1 for evidence in top['evidence'] if 'primary issue term match' in evidence or 'exact symbol match' in evidence)",
        "        def display_score(item: dict) -> float:",
        "            path = item['path'].lower()",
        "            penalty = 14.0 if '/test' in path or 'test' in path.split('/')[-1] or 'spec' in path.split('/')[-1] else 0.0",
        "            return item['score'] - penalty",
        "        top_display_score = display_score(top)",
        "        for item in ranked[1:4]:",
        "            item_primary_hits = sum(1 for evidence in item['evidence'] if 'primary issue term match' in evidence or 'exact symbol match' in evidence)",
        "            item_display_score = display_score(item)",
        "            score_gap = top_display_score - item_display_score",
        "            close_threshold = max(2.0, top_display_score * 0.2)",
        "            if score_gap <= close_threshold:",
        "                reasons = [f\"close runner-up by rank-adjusted score ({item_display_score:.1f} vs {top_display_score:.1f}); verify this candidate before editing the top-ranked file\"]",
        "            else:",
        "                reasons = [f\"lower rank-adjusted score than top candidate ({item_display_score:.1f} vs {top_display_score:.1f})\"]",
        "            if item_primary_hits < top_primary_hits:",
        "                reasons.append('weaker match to primary issue terms')",
        "            if top.get('related_tests') and not item.get('related_tests'):",
        "                reasons.append('no directly matched related test')",
        "            if item['path'] == top['path'] and item.get('symbol') != top.get('symbol'):",
        "                reasons.append('same file, but less specific symbol evidence')",
        "            decisions.append({",
        "                'path': item['path'],",
        "                'symbol': item.get('symbol'),",
        "                'score': item['score'],",
        "                'reason': '; '.join(reasons),",
        "                'evidence_excerpt': item['evidence'][:3],",
        "            })",
        "        return decisions",
        "",
        "    def root_cause_candidates_for(ranked: list[dict]) -> list[dict]:",
        "        root_causes: dict[tuple[str, str], dict] = {}",
        "        for source in ranked[:5]:",
        "            if not source.get('symbol'):",
        "                continue",
        "            if '/test' in source['path'].lower() or 'test' in source['path'].split('/')[-1].lower() or 'spec' in source['path'].split('/')[-1].lower():",
        "                continue",
        "            for row in conn.execute(",
        "                \"\"\"",
        "                SELECT tf.path, ts.name, ts.start_line, ts.end_line, e.confidence",
        "                FROM symbols ss",
        "                JOIN files sf ON sf.id = ss.file_id",
        "                JOIN edges e ON e.source_id = ss.id AND e.source_type = 'symbol' AND e.kind = 'CALLS'",
        "                JOIN symbols ts ON ts.id = e.target_id",
        "                JOIN files tf ON tf.id = ts.file_id",
        "                WHERE sf.path = ? AND ss.name = ?",
        "                LIMIT 8",
        "                \"\"\"",
        "                , (source['path'], source['symbol']),",
        "            ):",
        "                target_name = row['name'].lower()",
        "                source_name = source['symbol'].lower()",
        "                adds_primary_term = any(term.lower() in target_name and term.lower() not in source_name for term in primary_terms if len(term) >= 3)",
        "                if not adds_primary_term:",
        "                    continue",
        "                key = (row['path'], row['name'])",
        "                item = root_causes.setdefault(key, {",
        "                    'path': row['path'],",
        "                    'symbol': row['name'],",
        "                    'score': 0.0,",
        "                    'line_range': [row['start_line'], row['end_line']],",
        "                    'evidence': [],",
        "                })",
        "                item['score'] += min(18.0, max(4.0, source['score'] * 0.75))",
        "                item['evidence'].append(f\"call graph: {source['symbol']} calls {row['name']} (confidence {row['confidence']:.2f})\")",
        "        return sorted(root_causes.values(), key=lambda item: item['score'], reverse=True)[:5]",
        "",
        ...
        "        \"\"\"",
        "    ):",
        "        if row['name'].lower() in issue_lower:",
        "            add(row['path'], row['name'], 4.0, f\"explicit symbol mention in issue: {row['name']}\", row['start_line'], row['end_line'])",
        "",
        "    for term in primary_terms[:8]:",
        "        like = f'%{term}%'",
        "        for row in conn.execute(",
        "            \"\"\"",
        "            SELECT f.path, s.name, s.start_line, s.end_line",
        ...
        "            , (like, like),",
        "        ):",
        "            add(row['path'], row['name'], 1.5, f'primary issue term match: {term}', row['start_line'], row['end_line'])",
        "",
        ...
        "",
        "    def candidate_rank_score(item: dict) -> float:",
        "        path = item['path'].lower()",
        "        penalty = 14.0 if '/test' in path or 'test' in path.split('/')[-1] or 'spec' in path.split('/')[-1] else 0.0",
        "        return item['score'] - penalty",
        "",
        "    ranked = sorted(candidates.values(), key=lambda item: (candidate_rank_score(item), item['score']), reverse=True)[:limit]",
        "    for item in ranked:",
        "        item['related_tests'] = related_tests_for(item)",
        "        item['characterization_test'] = None",
        "        if item['related_tests']:",
        "            test_actions = [item['related_tests'][0]['command']]",
        "        else:",
        "            characterization = characterization_test_for(item)",
        "            item['characterization_test'] = characterization",
        "            test_actions = [",
        "                f\"add characterization test {characterization['path']}::{characterization['name']}\",",
        "                f\"reproducer input: {characterization['input']}\",",
        "                f\"expected assertion: {characterization['assertion']}\",",
        ...
        "    unknowns = build_unknowns(ranked)",
        "    if root_causes:",
        "        unknowns.append('Root-cause candidates use heuristic CALLS graph evidence; verify the runtime path before editing.')",
        "    return {",
        "        'issue': issue,",
        "        'parsed_files': files,",
        "        'terms': terms,",
        ...
        "        'unknowns': unknowns,",
        "        'why_not_other_candidates': why_not_other_candidates(ranked),",
