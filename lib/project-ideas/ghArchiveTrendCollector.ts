...
export function buildGhArchiveTrendQuery(input: CollectGhArchiveTrendsInput) {
  ...
  const query = [
    ...
    "  (COUNTIF(type = 'WatchEvent') * 10)",
    "    + (COUNTIF(type = 'ForkEvent') * 4)",
    "    + (COUNTIF(type = 'IssuesEvent') * 3)",
    "    + (LEAST(COUNTIF(type = 'PushEvent'), 100) * 0.1) AS trendScore",
    "FROM events",
    "GROUP BY repoFullName",
    "HAVING",
    "  COUNTIF(type = 'WatchEvent') > 0",
    "  OR COUNTIF(type = 'ForkEvent') > 0",
