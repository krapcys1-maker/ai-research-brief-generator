...
async function rankGoldQuery(goldQuery: GoldQuery) {
...
describe("retrieval gold benchmark suite", () => {
  ...
  it("keeps top-1 accuracy high across gold queries", async () => {
    const results = await Promise.all(
      goldQueries.map(async (goldQuery) => {
        const { selected } = await rankGoldQuery(goldQuery);
        return goldQuery.expectedTopIds.includes(selected[0]?.id ?? "");
      })
    );
    const accuracy =
      results.filter(Boolean).length / Math.max(1, results.length);
